...
@onready var filter_bar: HBoxContainer   = $LeftVBox/FilterBar  
...
@onready var inv_root := get_parent()  # 👈 one level up: Inventory_chest
...
func _ready():
	print("✅ Player_Panel script loaded:", self.name)
	await get_tree().process_frame  # Waits one frame
	print("inv_scroll now:", inv_scroll != null)  # Should now be true
	_setup_scroll()
	...
	print("👀 Setting debug colors for Player_Panel")
	inv_grid.add_theme_color_override("panel", Color(0.5, 0, 0, 0.2))  # Semi-transparent red
	inv_scroll.add_theme_color_override("panel", Color(0, 0.5, 0, 0.2))  # Semi-transparent green
	inv_grid.mouse_filter = Control.MOUSE_FILTER_IGNORE
	inv_scroll.mouse_filter = Control.MOUSE_FILTER_IGNORE
...
func _refresh_grid() -> void:
	...
	print("👀 Player inventory has:", items.size(), "items")
	...
	for s in items:
		...
		print("🔌 Connecting Player_Panel _on_slot_clicked, result:", result)

		# Optional: Inline signal listener for backup/extra debug
...
func _on_slot_clicked(slot: ItemSlot, shift: bool) -> void:
	if shift:
		...
		var to_panel = get_parent().get_node("Container_control/Storage_Panel")

		if to_panel == null:
			push_warning("❌ Could not find Storage_Panel for transfer.")
			return
...
func _close_loot_interface():
	get_tree().paused = false

	var transfer := get_tree().root.get_node_or_null("LocalMap/NodeTransferWindow")
	if transfer:
		transfer.queue_free()
...
func _save_inventory() -> void:
	print("💾 Saving", inventory_source_type, "with keys:", _inventory.keys())
	...
	match inventory_source_type:
		...
		"storage":
			if biome_id == "":
				push_warning("❌ Missing biome_id for storage save!")
				return
			LoadHandlerSingleton.save_storage_register(biome_id, _inventory)
