extends Panel
#this script is attached to Left_Panel, child of parent Inventory_mini (control) res://ui/scripts/Left_Panel_Inv_mini.gd
...
func _ready():
	print("✅ Inventory_mini is now active!")
...
func set_inventory(inv: Dictionary) -> void:
	if inv.has("inventory") and inv["inventory"] is Dictionary:
		push_warning("⚠️ set_inventory got WRAPPED dict! Unwrapping…")
		_inventory = inv["inventory"].duplicate(true)
	else:
		_inventory = inv.duplicate(true)

	# ✅ Normalize each stack to ensure correct weight/value
...
func _refresh_grid() -> void:
	...
	inv_grid.columns = 5  # or calculate dynamically
...
func _load_inventory_from_save() -> void:
	var raw := LoadHandlerSingleton.load_player_inventory_dict()
	if raw.has("inventory") and raw["inventory"] is Dictionary:
		push_warning("⚠️ _load_inventory_from_save got WRAPPED dict! Unwrapping…")
		_inventory = raw["inventory"]
	else:
		_inventory = raw
...
func _save_inventory() -> void:
	...
	if _inventory.has("inventory") and _inventory["inventory"] is Dictionary:
		...
		_inventory = _inventory["inventory"]
