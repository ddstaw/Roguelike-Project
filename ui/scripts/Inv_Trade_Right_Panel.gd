...
@onready var filter_bar: HBoxContainer   = $RightVBox/FilterBar  # ✅ This is still correct if LeftVBox is a child of Left_Panel
...
func set_inventory(inv: Dictionary) -> void:
	_inventory = inv.duplicate(true)  # Duplicate to avoid modifying the original dict
